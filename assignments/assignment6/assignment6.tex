...
\begin{itemize}
    \item Every \textit{rid} has a (\textit{title, total-duration, min-price})
    \item Every recipe has at least one step
    \item Every (\textit{rid, step-order}) determines a unique (\textit{description, duration})
    \item Every recipe step has at least one ingredient
    \item \textit{amount} is determined by (\textit{rid, step-order, ingredient})
    \item Every \textit{UPC} uniquely identifies a (\textit{producer, quantity, price, unit-price})
    \item \textit{ingredient} can have many \textit{UPC}
    \item \textit{producer} can produce many \textit{UPC}
\end{itemize}
