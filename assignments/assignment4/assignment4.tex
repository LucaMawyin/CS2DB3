...
$\text{MANU\_RECIPES}:=\pi_{rid,\text{manufacturer}}(\textbf{Ingredient}\bowtie_\text{Ingredient.ingredient=Product.ingredient}\textbf{Product})$\\
$\text{MUTLI\_INGREDIENT}:=\pi_{P_1.\text{ingredient}\mapsto\text{ingredient}}(\sigma_{P_1.\text{ingredient}=P_2.\text{ingredient}\land{P_1.\text{manufacturer}\neq{P_2.\text{manufacturer}}}}\\
(\rho_{P_1}(\textbf{Product}))\times\rho_{P_2}(\textbf{Product}))$
$\text{AVOID\_MANUFACTURER}$
